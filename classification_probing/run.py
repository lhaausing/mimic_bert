    ...
    with torch.no_grad():
        ...
        # no get_val_snippets, directly get 512 from bert
            ...
            for i in range(len(input_ids)): 
                ...
                non_pads=[(id!=1).int() for id in input_id]
                length=int(sum(non_pads).detach().numpy())
            ...
            if args.probing:
                if len(all_layers)>0:
                    all_layers=[all_layers[i]+[layers[i]] for i in range(len(layers))]
                    # all_logits.append(some_logits)
                else:
                    all_layers=[[layers[i]] for i in range(len(layers))]
                    # all_logits.append(some_logits)
        ...
        if args.probing:
            pickle.dump({"all_layers":all_layers}, open(args.checkpt_path+'_'+args.model_name+'_probing_val.pkl','wb'))
...
def train(args, train_loader, val_loader):
    ...
    model = snippet_model(args.model_name,n_class = 50,probing=args.probing)
    ...
    for i in range(start_epoch, args.n_epochs):
        ...
        for idx, (input_ids, attn_masks, labels) in tqdm(enumerate(train_loader)):
            ...
            for j in range(len(input_ids)):
                ...
                non_pads=[(id!=1).int() for id in input_id]
                # print(non_pads)
                length=sum(non_pads).detach().numpy()
                # print(1,length)
                length=int(sum(non_pads).detach().numpy())
                # print(2,length,length.type)
                # sys.stdout.flush()
        ...
        if args.probing:
            pickle.dump({"all_layers":all_layers}, open(args.checkpt_path+'_'+args.model_name+'_probing_train.pkl','wb'))
...
def main():
    ...
    pickle.dump({"train":train_loader,"val":val_loader}, open(args.checkpt_path+'_'+args.model_name+'_dataloader.pkl','wb'))
